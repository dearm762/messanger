...
if ($check_chat_result === false) {
    ...
    $sql = "SELECT * FROM messages WHERE chat_id='$chat_id' and (sender='$user1' or sender='$user2')";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $last = $row['message_id'] == $lastId ? true : false;
            list($date_last_mess, $time_last_mess) = explode(" ", $row['date']);
            $message = ['message_id' => $row['message_id'], 'sender' => $row['sender'], 'date' => $date_last_mess, 'date_time' => $time_last_mess, 'message' => $row['message'], 'read_status' => $row['read_status'], 'last'=>$last];
            $messages[] = $message;
        }
